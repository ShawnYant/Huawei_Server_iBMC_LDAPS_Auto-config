...
def configure_ldap_controller(ibmc_ip, token, ldap_server, ldap_port, user_domain, bind_dn, bind_pw, cert_verify_enabled, cert_verify_level, group_id, group_name, group_domain, group_role, group_interfaces):
    ...
    get_headers = {"X-Auth-Token": token}
    body, resp_headers = make_request(url, method='GET', headers=get_headers)
    if body is None or resp_headers is None:
        print(f"{ibmc_ip} 获取LDAP控制器配置失败，请检查API路径是否正确。")
        return
    etag = resp_headers.get('ETag') if resp_headers else None
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        bind_dn = sys.argv[7] if len(sys.argv) > 7 else "CN=ibmccloud,OU=cloud,DC=hkunicom,DC=com"
        bind_pw = sys.argv[8] if len(sys.argv) > 8 else "Cug@2027"
        # 强制使用CN=Users,DC=hkunicom,DC=com作为用户应用文件夹
        user_domain = "CN=Users,DC=hkunicom,DC=com"
        ...
        group_domain = sys.argv[13] if len(sys.argv) > 13 else "CN=hk,OU=cloud,DC=hkunicom,DC=com"
        ...
        ibmc_username = input("iBMC用户名: ") or "Administrator"
        ibmc_password = getpass.getpass("iBMC密码: ") or "Cug#hk2024aicc"
        ldap_server = input("LDAP服务器地址 (IP或域名): ") or "172.18.88.247"
        ldap_port = input("LDAP端口 (默认636): ") or "636"
        user_domain = input("用户域/UserDomain (默认 CN=Users): ") or "CN=Users"
        bind_dn = input("绑定DN (默认 CN=ibmccloud,OU=cloud,DC=hkunicom,DC=com): ") or "CN=ibmccloud,OU=cloud,DC=hkunicom,DC=com"
        bind_pw = getpass.getpass("绑定密码: ") or "Cug@2027"
        # 强制使用CN=Users,DC=hkunicom,DC=com作为用户应用文件夹
        user_domain = "CN=Users,DC=hkunicom,DC=com"
        ...
        group_domain = input("组域/GroupDomain (默认 CN=hk,OU=cloud,DC=hkunicom,DC=com): ") or "CN=hk,OU=cloud,DC=hkunicom,DC=com"
